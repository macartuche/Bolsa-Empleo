...
class UsersViewUsers extends JView
{
	function display($tpl = null)
	{
		...
		if (isset( $search ) && $search!= '')
		{
			...
			$where[] = 'a.username LIKE '.$searchEscaped.' OR a.email LIKE '.$searchEscaped.' OR a.name LIKE '.$searchEscaped." OR r.cedula = '".$search."'";
		}
		if ( $filter_type )
		{
		...
		//macf patch 
		if(empty($where))
			$where .=" where a.id = r.memberid";
		else
			$where .=" and a.id = r.memberid";

		$query = 'SELECT COUNT(a.id)'
		. ' FROM #__users AS a, #__jobs_resumes AS r'
		. $filter
		. $where;
		...
		/*$query = 'SELECT a.*, g.name AS groupname'
			...
			//agregado MACF-WUTOLOJA
			//. ' INNER JOIN #__jobs_resumes AS r ON r.memberid = a.id'
		*/	
		$query = 'SELECT a.* '
				 .'FROM #__users AS a '
				 .'INNER JOIN #__jobs_resumes AS r ON r.memberid = a.id '
