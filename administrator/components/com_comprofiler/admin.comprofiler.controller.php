...
* @version $Id: admin.comprofiler.controller.php 955 2010-03-05 21:19:33Z beat $
...
if ( file_exists( $tgzFile ) ) {
	if ( in_array( $taskPart1, array( 'showusers', 'showconfig', 'showTab', 'showField', 'showLists', 'tools', 'showPlugins', '' ) ) ) {
		...
		. ' <a href="index2.php?option=com_comprofiler&task=finishinstallation">' . CBTxt::Th('please click here to continue next and last installation step') . '</a>.</div>';
	}
}
...
function saveList( $option ) {
	...
	cbRedirect( "index2.php?option=$option&task=showLists", sprintf(CBTxt::T('Successfully Saved List: %s'), $row->title) );
}
...
function saveTab( $option ) {
	...
	$row->checkin();
	cbRedirect( "index2.php?option=$option&task=showTab", CBTxt::T('Successfully Saved Tab') . ": ". $row->title );
}
...
function saveUser( $option ) {
	...
	cbRedirect( "index2.php?option=$option&task=showusers", sprintf(CBTxt::T('Successfully Saved User: %s'), $userComplete->username) );
}
...
function saveConfig ( $option ) {
	...
	if ( $adminFS->isUsingStandardPHP() && ! is_writable($configfile)) {
		...
		if (!is_writable($configfile)) {
			cbRedirect("index2.php?option=$option", CBTxt::T('FATAL ERROR: Config File Not writeable') );
		}
	}
	...
	if ( $result !== false ) {
		...
		cbRedirect( "index2.php?option=$option", $msg . ': ' . CBTxt::T('ERROR: Configuration file administrator/components/com_comprofiler/ue_config.php could not be written by webserver. Please change file permissions in your web-pannel.') );
	}
}
...
function checkcbdb( $dbId = 0 ) {
	...
	if ( $dbId == 0 ) {
		...
		echo "<div style='text-align:left;'><p>". CBTxt::T('Checking Community Builder Datbase') .":</p>";
		...
		if ( $bad_rows === null ) {
			...
			echo '<p><font color=red>' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="index2.php?option=com_comprofiler&amp;task=fixcbmiscdb&amp;' . $cbSpoofField . '=' . $cbSpoofString . '"> ' . CBTxt::T('then by clicking here') . '</a>.</font></p>';
		} else {
		...
		echo "<div style='text-align:left;'><p>" . CBTxt::T('Checking Users Datbase') . ":</p>";
		...
		if ( $bad_rows === null ) {
			...
			echo "<p><font color=red>" . sprintf(CBTxt::T('Warning: %s entries in the users table without corresponding core_acl_aro table rows.'), count($bad_rows)) . "</font></p>";
			$badids	=	array();
			foreach ($bad_rows as $bad_row) {
				$badids[(int) $bad_row->id]	=	$bad_row->id;
			}
			echo "<p><font color=red>" . sprintf(CBTxt::T('user id: %s are missing in core_acl_aro table'), implode( ', ', $badids )) . ( isset( $badids[0] ) ? " " . CBTxt::T('This user entry with id 0 should be removed, as it\'s not allowed.') : "" ) . "</font></p>";
			echo '<p><font color=red>' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="index2.php?option=com_comprofiler&amp;task=fixacldb&amp;' . $cbSpoofField . '=' . $cbSpoofString . '">' . CBTxt::T('then by clicking here') . '</a>.</font></p>';
		} else {
			echo "<p><font color=green>" . CBTxt::T('All users table rows have ACL entries in core_acl_aro table.') . "</font></p>";
		}
		...
		if ( $bad_rows === null ) {
			...
			echo "<p><font color=red>" . sprintf(CBTxt::T('Following entries of core_acl_aro table are missing in users table: %s.'), implode( ', ', $badids )) . ( isset( $badids[0] ) ? "<br /> " . CBTxt::T('This core_acl_aro entry with (user) value 0 should be removed, as it\'s not allowed.') : "" ) . ( ( $bad_row->aro_id == 0 ) ? " " . CBtxt::T('This core_acl_aro entry with aro_id 0 should be removed, as it\'s not allowed.') : "" ) . "</font></p>";
			echo '<p><font color=red>' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="index2.php?option=com_comprofiler&amp;task=fixacldb&amp;' . $cbSpoofField . '=' . $cbSpoofString . '">' . CBTxt::T('then by clicking here') . '</a>.</font></p>';
		} else {
			echo "<p><font color=green>" . CBTxt::T('All ACL core_acl_aro table rows have corresponding entries in users table.') . "</font></p>";
		}
		...
		if ( $bad_rows === null ) {
			...
			echo '<p><font color=red>' . CBTxt::T('This one can be fixed by <strong>first backing up database</strong>') . ' <a href="index2.php?option=com_comprofiler&amp;task=fixacldb&amp;' . $cbSpoofField . '=' . $cbSpoofString . '">' . CBTxt::T('by clicking here') . '</a>.</font></p>';
		} else {
			echo "<p><font color=green>" . CBTxt::T('All core_acl_aro table rows have ACL entries in core_acl_groups_aro_map table.') . "</font></p>";
		}
...
function editPlugin( $option, $task, $uid) {
	...
	if ($row->checked_out && $row->checked_out <> $_CB_framework->myId() ) {
		echo "<script type=\"text/javascript\">alert('" . addslashes( sprintf(CBTxt::T('The plugin %s is currently being edited by another administrator'), $row->name) ) . "'); document.location.href='index2.php?option=$option'</script>\n";
		exit(0);
	}
...
function pluginMenu( $option, $uid) {
	...
	if ( ! $uid ) {
		echo "<script type=\"text/javascript\">alert('" . addslashes( CBTxt::T('No plugin selected') ) . "'); document.location.href='index2.php?option=$option'</script>\n";
		exit(0);
	}
	...
	// get params values
		...
		echo "<script type=\"text/javascript\">alert('" . addslashes( sprintf(CBTxt::T('The plugin %s has no administrator file $phpfile'), $row->name) ) . "'); document.location.href='index2.php?option=$option'</script>\n";
		exit(0);
	}
}
		...
		foreach ( $cid as $id ) {
			...
			if ( $row->load( (int) $id ) ) {
				if ( ( $row->type == "language" ) && $row->published ) {
					cbRedirect( 'index2.php?option='. $option .'&task=showPlugins', CBTxt::T('Language plugins cannot be unpublished, only uninstalled'), 'error' );
				} elseif ( ( $row->id == 1 ) && $row->published ) {
					cbRedirect( 'index2.php?option='. $option .'&task=showPlugins', CBTxt::T('Core plugin cannot be unpublished'), 'error' );
				}
			} else {
				cbRedirect( 'index2.php?option='. $option .'&task=showPlugins', CBTxt::T('Plugin can not be found'), 'error' );
			}
		}
	}
	$cids = implode( ',', $cid );
...
//	$userfile['name']		=	stripslashes( $userfile['name'] );
	...
	if ( $resultdir !== false ) {
		...
		$installer->cleanupInstall( $localName, null );
